// FINAL MUTLAK server.js - FIX MONGODB, ASYNC MODULE LOAD, DAN VERCEL EXPORT
const express = require('express');
const mongoose = require('mongoose');
const bcrypt = require('bcryptjs');
const jwt = require('jsonwebtoken');
const axios = require('axios');
// TIDAK PERLU { Readable } dari 'stream' karena tidak dipakai
...
// TIDAK PERLU PORT karena Vercel yang urus
...
async function startServer() {
    ...
    try {
        ...
        // Biarkan server jalan, tapi API chat akan gagal
    }
    ...
    // Middleware untuk file statis di Vercel, biasanya tidak perlu
    // app.use(express.static('public'));
    
    // =ASUMSI: Vercel menangani routing file statis public/ secara terpisah=
    ...
    try {
        ...
        console.error('🚨 MONGODB CONNECTION ERROR: Harap cek Environment Variable MONGODB_URI dan IP Access di MongoDB Atlas!', err);
    }
    ...
    app.post('/api/register', async (req, res) => {
        if (mongoose.connection.readyState !== 1) {
             // FIX ERROR: Pesan error lebih jelas
             return res.status(503).send({ error: 'DB_CONNECTION_FAILED: Cek MONGODB_URI dan IP Access di MongoDB Atlas. Log Vercel harusnya berisi error koneksi.' });
        }
        try {
    ...
    app.post('/api/login', async (req, res) => {
        if (mongoose.connection.readyState !== 1) {
             return res.status(503).send({ error: 'DB_CONNECTION_FAILED: Cek MONGODB_URI dan IP Access di MongoDB Atlas.' });
        }
        try {
    ...
    app.post('/api/chat', auth, async (req, res) => {
        ...
        // Cek inisialisasi AI (di luar try catch)
        if (!ai) {
             return res.status(500).end(`❌ Error: Gemini AI module failed to load. Check GEMINI_API_KEY.`);
        }
    ...
    app.post('/api/midtrans/token', auth, async (req, res) => {
        ...
        if (!snap) {
             return res.status(500).json({ error: 'Midtrans module failed to load. Check MIDTRANS_SERVER_KEY.' });
        }
    ...
    app.post('/api/midtrans/notification', async (req, res) => {
         if (!core) {
             return res.status(500).send('Midtrans Core module failed to load.');
        }
        try {
    ...
    // Rute default untuk mengatasi 404 jika ada request non-api yang masuk ke server.js
    app.use((req, res) => {
        if (req.path.startsWith('/api/')) {
            res.status(404).json({ error: 'API Endpoint Not Found' });
        } else {
             // Asumsi file statis ditangani Vercel, jika tidak berhasil, beri 404
            res.status(404).send('File Not Found');
        }
    });
}

// Panggil fungsi startServer dan export app
